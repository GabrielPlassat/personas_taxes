...
st.title("Simulateur de mobilite decarbonee")
st.caption(
    f"Horizon {horizon}  |  Scenario VE : {scenario_ve}  |  "
    f"L6/L7 : {part_l6l7:.0%}  |  Taxe : {taxe_km} c EUR/km  |  "
    f"Tarif : {scenario_tarif}"
)
st.divider()
...
col1.metric(
    label="VP electriques",
    value=f"{metr['part_ve']:.0%}",
    help="Part des VP electriques dans le parc simule"
)
col2.metric(
    label="CO2 evite/an",
    value=f"{metr['co2_evite_mt']:.1f} Mt",
    help="Millions de tonnes CO2 evitees a l'echelle France"
)
col3.metric(
    label="Cout mobilite/mois",
    value=f"{metr['cout_total']:.0f} EUR",
    help="Cout moyen mensuel par conducteur (energie + taxe km)"
)
col4.metric(
    label="Balance fiscale",
    value=f"{metr['balance_fiscale']:+.1f} Md EUR/an",
    delta=f"Taxe {metr['recettes_taxe']:.1f} Md EUR — TICPE -{metr['perte_ticpe']:.1f} Md EUR",
    delta_color="normal" if metr['balance_fiscale'] >= 0 else "inverse",
    help="Recettes taxe km moins perte TICPE"
)
col5.metric(
    label="Arbitrage modal",
    value=f"{metr['pct_bascule']:.1%}",
    help="Part des personas dont le cout depasse le seuil d'arbitrage"
)
...
with tab2:
    ...
    with col_a:
        ...
        try:
            ...
            # Fenêtre solaire
            fig3.add_vrect(x0=11, x1=15, fillcolor='rgba(245,166,35,0.18)',
                           line_width=0, annotation_text="Pic solaire")
            # Fenêtre nuit
            fig3.add_vrect(x0=22, x1=24, fillcolor='rgba(0,119,182,0.12)',
                           line_width=0)
            fig3.add_vrect(x0=0, x1=6, fillcolor='rgba(0,119,182,0.12)',
